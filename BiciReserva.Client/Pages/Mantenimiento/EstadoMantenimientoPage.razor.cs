...
{
    public partial class EstadoMantenimientoPage
    {
        ...
        {
            insert = true;
            var response = await Repository.Delete($"api/v1/StateMaintenance/{estado.idEstado}");
        ...
        async Task OnCreateRow(GStateGeneralDTO estado)
        {
            ...
            var response = await Repository.Post<GEstadoDTO, GStateGeneralDTO>("api/v1/StateMaintenance", state);
        ...
        {
            insert = true;
            var response = await Repository.Put<GStateGeneralDTO>($"api/v1/StateMaintenance", estado);
